<template lang='pug'>
  .contain.py2
    ...
    .text(v-if='!stratrunners.length')
      p Bundle does not have any stratrunners.
    ...
    h3(v-if='isAdmin && false') Watchers
    .text(v-if='!watchers.length && false')
      p Bundle does not have any watchers. COMING SOON! Need to add bundleUuids array to watchers created
    table.full.clickable(v-if='watchers.length && isAdmin  && false')
      thead
        tr
          th exchange
          th currency
...
export default {
  ...
  props: [ 'bundle', 'bundleUuid' ],
  computed: {
    stratrunners: function() {
      const bundleGekkos = _.filter(this.$store.state.gekkos, g => g.config && g.config.bundleUuid === this.bundleUuid);
      const bundleArchivedGekkos = _.filter(this.$store.state.archivedGekkos, g => g.config && g.config.bundleUuid === this.bundleUuid);
      return _.orderBy(bundleGekkos, g=>g.logType !== 'tradebot')
        .concat(_.orderBy(bundleArchivedGekkos, g=>g.logType !== 'tradebot'))
          .filter(g => {
    ...
    watchers: function() {
      const bundleGekkos = _.filter(this.$store.state.gekkos, g => g.config && g.config.bundleUuid === this.bundleUuid);
      const bundleArchivedGekkos = _.filter(this.$store.state.archivedGekkos, g => g.config && g.config.bundleUuid === this.bundleUuid);
      return _.values(bundleGekkos)
        .concat(_.values(bundleArchivedGekkos))

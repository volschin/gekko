...
strat.init = function() {
  ...
  this.BACKTEST_TAKE_PROFIT = this.settings.BACKTEST_TAKE_PROFIT; // 10% using this to make BTs less dependant on HUGE swings
...
strat.check = function(candle) {
  if (this.BACKTEST_TAKE_PROFIT && this.advised && this.candle.close > this.buyPrice * (1 + this.BACKTEST_TAKE_PROFIT)) {
      this.sell(`sell signal for BACKTEST_TAKE_PROFIT`);
  } else if (this.BACKTEST_TAKE_PROFIT && this.advisedShort && this.candle.close < this.buyPrice * (1 - this.BACKTEST_TAKE_PROFIT)) {
    this.sell(`sell signal for BACKTEST_TAKE_PROFIT, short`
      , { margin: { type: 'short', limit: 1 } });
  } else {
    if (this.useDependencies) {
      if (this.dependencyLast === 'fastLong' || this.dependencyLast === 'slowLong') {
        this.buy(`buy signal for long trade, currentTrend: ${currentFrame.currentTrend}`);

...
var strat = {
  ...
  check: function()
  {
    ...
    if( maFast < maSlow )
    {
      ...
      // ADX trend strength?
      rsi_mod_hi = this.BEAR_MOD_high;
      rsi_mod_low = this.BEAR_MOD_low;

      // BB persistence
      this.BBpersistence = this.settings.BBtrend.bearPersistence;
    ...
    else
    {
      ...
      */
      // ADX trend strength?
      rsi_mod_hi = this.BULL_MOD_high;
      rsi_mod_low = this.BULL_MOD_low;

      // BB persistence
      this.BBpersistence = this.settings.BBtrend.bullPersistence;
    ...
    if( adx > this.settings.ADX.high ) {
      rsi_hi = rsi_hi + rsi_mod_hi;
      rsi_low = rsi_low + rsi_mod_low;
    }


    if( rsi < rsi_low && this.BBtrend.zone == 'low' && this.BBtrend.duration >= this.BBpersistence ) this.long();

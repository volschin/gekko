...
strat.init = function() {
  ...
  let currentCandle, currentPrice = 0.0, buyPrice = 0.0, advised = false, tradeInitiated = false, buyTs, trailingStop = this.settings.trailingStop;
...
// What happens on every new candle?
  ...
  this.buy = function(reason) {
    ...
    if (!tradeInitiated) { // Add logic to use other indicators
      consoleLog(`strat.buy:: run, trailingStop=${ trailingStop }%`);
      this.notify({
        type: 'buy advice',
        reason: reason,
      });

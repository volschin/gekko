...
strat = {
  ...
  update: function (candle) {
    ...
    if (this.HistoricStickPattern.length > 10) { this.HistoricStickPattern.shift(); }
    else { for (i = 0; i < 9; i++) { this.HistoricStickPattern.push(Pattern); } }; // not ideal but not sure how to do it otherwise
  ...
  check: function (candle) {
    ...
    if (!this.Gotcoins) {
      ...
      else if (maShort + this.Toml.ShortSight < predictShort && this.CurrentCandle.close < this.PreviousCandle.close) { this.short('ShortSight   sell ', this.color.Cyan) }
  ...
  short: function (msg, kleur = this.color.Default) {
    ...
    if (this.Gotcoins) {
      ...
      console.log(trend + kleur + msg + prof + '  ' + this.color.Red + this.CurrentCandle.start.format('YYYY MMM DD hh:mm'));

...
var nndatafile = "D:\\projects\\bot\\gekko-fork\\nn_files\\" + 'trained.js'; // the nn state needs to be saved between runs this is the location for it
...
var strategy = {
  ...
  onTrade: function(event) {
    ...
    this.prevPriceBuy = event.price + diff;
    console.log(`onTrade: this.prevPrice: ${ this.prevPrice }, this.prevPriceBuy: ${ this.prevPriceBuy }`);
  },
  ...
  check : function(candle) {
    if (this.predictionCount > this.settings.min_predictions) {
      if (this.prevAction === 'buy' && this.settings.stoploss_enabled && this.indicators.stoploss.action === 'stoploss') {
      ...
      let currentPrice = candle.close;    // bid (you sell)
      let currentPriceBuy = candle.close + diff; // ask (you buy )
      let meanp = math.mean(prediction, currentPrice);
      let meanpBuy = math.mean(prediction, currentPriceBuy);
      let meanAlpha = (meanp - currentPrice) / currentPrice * 100;
      let meanAlphaBuy = (meanpBuy - currentPrice) / currentPriceBuy * 100;
      ...
      let signalSell = candle.close > this.prevPriceBuy || candle.close < (this.prevPriceBuy * this.hodle_threshold);

      let signal = meanp < currentPrice;
      let signalBuy = meanpBuy < currentPriceBuy;
      if ( this.prevAction !== 'buy' && signalBuy === false  && meanAlphaBuy > this.settings.threshold_buy ) {
        log.debug("Buy - Predicted variation: ", meanAlphaBuy);
        console.log(`check: meanpBuy: ${ meanpBuy }, currentPriceBuy: ${ currentPriceBuy }, meanAlphaBuy: ${ meanAlphaBuy }, signalBuy: ${ signalBuy }`);
        return this.advice('long');
      } else if (this.prevAction !== 'sell' && signal === true && meanAlpha < this.settings.threshold_sell && signalSell) {
